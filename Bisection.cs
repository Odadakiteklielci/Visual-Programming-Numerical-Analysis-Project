...
namespace Sayısal_Analiz_Visual_Proje_
{
    public partial class Bisection : Form
    {
        ...
        public Bisection(UserSQL userSQL)
        {
            InitializeComponent();
        ...
        public void createDatabase()
        {
            if (!File.Exists(dbName))
            {
                ...
                command.CommandText = "CREATE TABLE bisectiongecmis(id INTEGER PRIMARY KEY AUTOINCREMENT, fonksiyon varchar(255) not null, a INTEGER not null, b INTEGER not null, sonuc DOUBLE not null, datetime DATETIME not null, username varchar(255) not null)";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                BisectionSQL bisectionSQL = null; 
                switch (comboBox1.SelectedIndex)
                {
                    case 0:
                        bisectionSQL = new BisectionSQL("2^x-5x+2", 0, 1, Hesapla(0.01, 100), DateTime.Now, userSQL.Username);
                        break;
                    case 1:
                        bisectionSQL = new BisectionSQL("e^x-3x", 0, 1, Hesapla(0.01, 100), DateTime.Now, userSQL.Username);
                        break;
                    case 2:
                        bisectionSQL = new BisectionSQL("x^2-2", 1, 2, Hesapla(0.01, 100), DateTime.Now, userSQL.Username);
                        break;
                    default:
                        bisectionSQL = new BisectionSQL("2^x-5x+2", 0, 1, Hesapla(0.01, 100), DateTime.Now, userSQL.Username);
                        break;
                ...
                if (bisectionSQL != null)
                {
                    ...
                    cmd.CommandText = @"INSERT INTO bisectiongecmis 
                                (fonksiyon, a, b, sonuc, datetime, username) 
                                VALUES 
                                (@fonksiyon, @a, @b, @sonuc, @datetime, @username)";
                    ...
                    cmd.Parameters.AddWithValue("@username", bisectionSQL.Username);
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception ex)
