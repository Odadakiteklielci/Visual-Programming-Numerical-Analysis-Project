...
namespace Sayısal_Analiz_Visual_Proje_
{
    public partial class Lagrange : Form
    {
        ...
        UserSQL UserSQL;
        public Lagrange(List<double> x, List<double> y, UserSQL userSQL)
        {
            InitializeComponent();
            this.x = x;
            this.y = y;
        ...
        public void createDatabase()
        {
            if (!File.Exists(dbName))
            {
                ...
                command.CommandText = "CREATE TABLE lagrangegecmis(id INTEGER PRIMARY KEY AUTOINCREMENT,noktalar varchar(255) not null, x DOUBLE not null, sonuc DOUBLE, datetime DATETIME not null, username varchar(255) not null)";
