    ...
    public partial class Sekant : Form
    {
        ...
        public void createDatabase()
        {
            if (!File.Exists(dbName))
            {
                ...
                command.CommandText = "CREATE TABLE sekantgecmis(id INTEGER PRIMARY KEY AUTOINCREMENT, fonksiyon varchar(255) not null, x0 DOUBLE not null,x1 DOUBLE not null,iterasyonSayisi INT not null, sonuc DOUBLE not null, datetime DATETIME not null, username varchar(255) not null)";
            ...
            try
            {
                ...
                switch (comboBox1.SelectedIndex)
                {
                    case 0:
                        ...
                        sekantSQL = new SekantSQL("lnx-e^x", x0, x1, iterat,x2, DateTime.Now,userSQL.Username);
                        break;
                    case 1:
                        x0 = 2.5;
                        x1 = 3;
                        SecantMethod(x0, x1, out x2, out iterat);
                        sekantSQL = new SekantSQL("x^3-20", x0, x1, iterat,x2, DateTime.Now,userSQL.Username);
                        break;
                    case 2:
                        x0 = 1;
                        x1 = 1.2;
                        SecantMethod(x0, x1, out x2, out iterat);
                        sekantSQL = new SekantSQL("lnx-cos(x)", x0, x1, iterat,x2, DateTime.Now, userSQL.Username);
                        break;
                    default:
                        x0 = 0.5;
                        x1 = 0.6;
                ...
                cmd.CommandText = @"INSERT INTO sekantgecmis 
                         (fonksiyon, x0, x1, iterasyonSayisi, sonuc, datetime,username) 
                         VALUES 
                         (@fonksiyon, @x0, @x1, @iterasyonSayisi, @sonuc, @datetime,@username)";
                ...
                cmd.Parameters.AddWithValue("@username", sekantSQL.Username);
